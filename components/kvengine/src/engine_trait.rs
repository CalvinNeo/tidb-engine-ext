...
impl StatisticsReporter<Engine> for EngineReporter {
    ...
    fn collect(&mut self, engine: &Engine) {
        panic!()
    }
...
impl MiscExt for Engine {
    ...
    fn flush_cfs(&self, _cfs: &[&str], _wait: bool) -> TraitsResult<()> {
        panic!()
    }
    ...
    fn locked(path: &str) -> TraitsResult<bool> {
        panic!()
    }
...
impl CfNamesExt for EngineSnapshot {
    fn cf_names(&self) -> Vec<&str> {
        panic!()
    }
}
